...
public class CzlowiekDzieciFactory {
  ...
  private static void randomizuj(ArrayList<Czlowiek> mezczyzni, ArrayList<Czlowiek> kobiety, Stack<Czlowiek> potomstwo) {
    ...
    for(int i=0 ; (i < min(mezczyzni.size(),kobiety.size())) && (!potomstwo.isEmpty()) ; i++)
    {
      ...
      {
        los = rand.nextInt(2);
        mezczyzni.get(i).dodajDziecko(potomstwo.getLast());
        kobiety.get(i).dodajDziecko(potomstwo.getLast());
        potomstwo.pop();
      }
  ...
  public static Set<Czlowiek> wypelnijDzieci(String filePath) {
    ...
    randomizuj(tatusie,mamusie,dzieci);
