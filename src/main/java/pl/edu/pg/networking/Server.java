    ...
    private class ClientHandler extends Thread {
            ...
            try {
                switch (message.prefix) {
                    ...
                    case RESPONSE:
                        ...
                        break;
                    case PROTOCOL:
                        logger.info("{} - Received PROTOCOL message", clientName);
                        sendResponse(Message.Response.OK);
                        handleProtocol();
                        break;
                }
        ...
        private Message recieveMessage() {
            try {
                ...
                logger.warn("Error receiving message from client", e);
                return null;
            }
        }
        ...
        private void handleProtocol() {
            ...
            if (message == null)
                return;
            sendMessage(Message.Prefix.PROTOCOL, Message.Protocol.DATA);
            message = recieveMessage(); // result
            if (message == null)
                return;
            logger.info("Checking protocol data...");
            try {
                Thread.sleep(500);
            } catch (InterruptedException e) {
                logger.error("Error during sleep: ", e);
            }
            sendMessage(Message.Prefix.PROTOCOL, Message.Protocol.END);
